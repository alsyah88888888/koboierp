...
export function SupplierModal({ onClose, onSuccess }: { onClose: () => void, onSuccess?: (vendor: any) => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert(error.message || "Terjadi kesalahan saat menyimpan data supplier.");
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
                <div className="p-6 border-b flex justify-between items-center bg-slate-50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-800">Tambah Supplier Baru</h2>
                            <p className="text-xs text-slate-500">Form input cepat master vendor / supplier</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-slate-600 uppercase">Nama Supplier *</label>
                        <input
                            required
                            autoFocus
                            value={formData.name}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-600 uppercase">Perwakilan / Email</label>
                            <input
                                type="email"
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-slate-600 uppercase">No. Telepon</label>
                            <input
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-slate-600 uppercase">Alamat</label>
                        <textarea
