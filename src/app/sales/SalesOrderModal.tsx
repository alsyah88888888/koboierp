...
import { X, Plus, Trash2, Save, Calculator, FileText, CheckCircle, ChevronRight, ShoppingCart, Tag, Loader2 } from "lucide-react";
...
export default function SalesOrderModal({ products, customers, warehouses, initialData, onClose }: SalesOrderModalProps) {
    ...
    // Calculation logic matching SalesModal
    ...
    const handleSubmit = async (isConfirm: boolean = false) => {
        if (!buyerName || items.some(i => !i.productId)) {
            setError("Data Tidak Lengkap. Mohon isi nama buyer dan semua produk.");
            return;
        }
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-6xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[95vh] overflow-hidden border border-slate-200 scale-in-center">
                ...
                {/* Header matching SalesModal style */}
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[9px] font-black rounded uppercase border border-indigo-100">PI (Penawaran)</span>
                                <ChevronRight className="h-3 w-3 text-slate-300" />
                                <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 text-[9px] font-black rounded uppercase border border-emerald-100">PO (Pesanan)</span>
                            </div>
                        </div>
                    </div>
                ...
                <form onSubmit={(e) => e.preventDefault()} className="flex-1 overflow-y-auto bg-slate-50/30 custom-scrollbar">
                    <div className="p-4 lg:p-8 space-y-8">
                        
                        {/* Guidance Alert */}
                        <div className="bg-amber-50 border border-amber-200 rounded-[2rem] p-6 flex items-start gap-5 shadow-sm">
                            <div className="bg-amber-500 text-white p-3 rounded-2xl shrink-0 shadow-lg shadow-amber-200">
                                <Calculator className="h-5 w-5" />
                            </div>
                            <div className="space-y-1">
                                <p className="text-[11px] font-black text-amber-900 uppercase tracking-widest">PANDUAN ADMIN:</p>
                                <p className="text-xs text-amber-800 leading-relaxed font-medium">
                                    Gunakan <span className="font-black underline">Simpan sebagai PI</span> jika baru sekedar penawaran (stok tidak terkunci).<br/>
                                    Gunakan <span className="font-black text-emerald-700 underline">Konfirmasi Jadi PO</span> jika pelanggan sudah OK, agar data bisa ditarik saat membuat <span className="font-black italic">Surat Jalan (SJ)</span>.
                                </p>
                            </div>
                        </div>

                        {/* Basic Info Grid matching SalesModal */}
                        ...
                        {/* Items Section matching SalesModal */}
                                    ...
                                    <div key={index} className="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-4 px-4 py-3 lg:p-2 bg-white lg:bg-white border border-slate-100 lg:border-slate-200 rounded-2xl lg:rounded-xl hover:border-indigo-300 transition-all group shadow-sm">
                                        <div className="w-full lg:flex-[4] min-w-0">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-slate-50 border border-transparent px-3 py-2 rounded-lg text-sm font-black outline-none focus:bg-white focus:border-indigo-500 transition-all"
                        ...
                        {/* Footer & Financial Summary matching SalesModal */}
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                                <div className="lg:col-span-2 hidden lg:block">
                                    <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 h-full flex flex-col justify-center items-center text-slate-400 shadow-sm">
                                        ...
                                        <p className="text-[11px] font-black uppercase tracking-[0.3em] text-center opacity-40">Dokumen Penawaran & Pesanan Siap Disimpan</p>
                                    </div>
                                </div>
                                    ...
                                    <div className="relative z-10 space-y-6">
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-[11px] font-bold text-orange-400 uppercase tracking-widest flex justify-between ml-1">
                                                Diskon Global
                                                <span 
                                                    className="text-[10px] lowercase opacity-40 cursor-pointer hover:opacity-100 transition-opacity"
                                                    onClick={async () => {
                                                        const p = await prompt({
                                                            title: "Diskon Persentase",
                                                            message: "Masukkan nilai persentase diskon global:",
                                            ...
                                            <div className="flex items-center bg-white/5 border border-white/10 rounded-2xl px-4 focus-within:border-orange-500/50 transition-all font-mono ring-offset-slate-900">
                                                <span className="text-slate-600 font-black mr-2 text-sm">Rp</span>
                                                <input
                                                    type="number"
                                                    value={totalDiscount}
                                                    onChange={e => setTotalDiscount(Number(e.target.value))}
                                                    className="w-full bg-transparent py-3 text-base font-black text-white outline-none"
                                                    placeholder="0"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex gap-3">
                                                <button
                                                    type="button"
                                                    onClick={() => { setIsPKP(true); setTaxRate(0.11); }}
                                                    className={`flex-1 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                                                        isPKP 
                                                            ...
                                                            : 'bg-white/5 border-white/10 text-slate-600 hover:border-indigo-500/50'
                                                    }`}
                                                >
                                                    PKP
                                                    <span className="block text-[8px] font-bold opacity-50 mt-1 uppercase">KB-PO • PPN 11%</span>
                                                </button>
                                                <button
                                                    type="button"
                                                    ...
                                                    className={`flex-1 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                                                        !isPKP 
                                                            ? 'bg-slate-700 border-slate-600 text-white shadow-xl shadow-slate-700/30' 
                                                            : 'bg-white/5 border-white/10 text-slate-600 hover:border-slate-500/50'
                                                    }`}
                                                >
                                                    Non PKP
                                                    <span className="block text-[8px] font-bold opacity-50 mt-1 uppercase">KB-PI • 0%</span>
                                                </button>
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-6 mt-4 border-t border-white/10">
                                            ...
                                            <p className="text-4xl font-black text-emerald-400 tracking-tighter leading-none mb-6 font-mono">Rp {grandTotal.toLocaleString('id-ID')}</p>
                                            
                                            <div className="flex flex-col gap-3">
                                                <button
                                                    onClick={() => handleSubmit(true)}
                                                    disabled={loading}
                                                    className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black shadow-2xl shadow-indigo-600/20 hover:bg-slate-800 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3 group"
                                                >
                                                    {loading ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <span className="text-xs tracking-[0.2em] uppercase">Konfirmasi Jadi PO</span>
                                                        </>
                                                    )}
                                                </button>
                                                <button
                                                    ...
                                                    disabled={loading}
                                                    className="w-full bg-white/5 text-slate-400 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2 border border-white/5"
                                                >
                                                    <Save className="h-4 w-4" /> Simpan sebagai PI (Draft)
                                                </button>
                                            </div>
                                        </div>
                                    </div>
