...
export function FinanceModal({ accounts, onClose }: { accounts: any[], onClose: () => void }) {
    ...
    // Account Selection
    const [bankAccountId, setBankAccountId] = useState(""); // The Asset/Bank account in COA
    const [targetAccountId, setTargetAccountId] = useState(""); // The Expense/Income/Category account

    const bankAccounts = accounts.filter(a => a.type === "ASSET");
    const categoryAccounts = accounts.filter(a => a.type !== "ASSET" || a.id !== bankAccountId);

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!bank || !amount || !targetAccountId || !bankAccountId) {
            setError("Mohon lengkapi data: Bank, Akun, dan Nominal.");
            return;
        }
                ...
                <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-8 space-y-6 bg-white">
                    ...
                    <div className="flex p-1.5 bg-slate-100 rounded-xl border-2 border-slate-200">
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-all ${type === "PAYMENT" ? "bg-white shadow-md text-red-600 border border-slate-200" : "text-slate-500 hover:text-slate-700"}`}
                        >
                            <ArrowUpRight className="h-4 w-4 text-red-600" />
                            <span className="text-red-600">Pembayaran (Keluar)</span>
                        </button>
                        <button
                            type="button"
                            onClick={() => setType("RECEIPT")}
                            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-bold transition-all ${type === "RECEIPT" ? "bg-white shadow-md text-emerald-600 border border-slate-200" : "text-slate-500 hover:text-slate-700"}`}
                        >
                            <ArrowDownLeft className="h-4 w-4" /> Penerimaan (Masuk)
                        </button>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Bank / Metode</label>
                            <input
                                value={bank}
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Pilih Akun Kas/Bank (COA)</label>
                            <select
                                value={bankAccountId}
                    ...
                    <div className="space-y-6 pt-6 border-t-2 border-slate-100">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Kategori / Akun Lawan</label>
                                <select
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Keterangan / Deskripsi</label>
                            <textarea
                                value={description}
                                onChange={e => setDescription(e.target.value)}
                                className="w-full bg-white border-2 border-slate-300 px-3 py-2.5 rounded-lg outline-none focus:border-primary transition-all font-medium min-h-[100px]"
