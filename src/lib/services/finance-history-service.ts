...
export async function getApprovalHistoryService(
    ...
    try {
        ...
        const invoiceNumbers = [...new Set(transactions.map((t: any) => t.invoiceNumber).filter(Boolean))] as string[];

        // Lookup SalesDelivery (Piutang / AR)
        const salesDeliveries = invoiceNumbers.length > 0 ? await (prisma as any).salesDelivery.findMany({
            where: {
                OR: [
        ...
        salesDeliveries.forEach((sd: any) => {
            sdMap.set(sd.deliveryNumber, sd);
            if (sd.invoiceNumber) sdMap.set(sd.invoiceNumber, sd);
        });

        // Lookup GoodsReceipt (Hutang / AP)
        const goodsReceipts = invoiceNumbers.length > 0 ? await (prisma as any).goodsReceipt.findMany({
            where: {
                OR: [
                    { receiptNumber: { in: invoiceNumbers } },
                    { formNumber: { in: invoiceNumbers } }
                ]
            },
        ...
        const result = transactions.map((t: any) => {
            ...
            const invoiceNo = t.invoiceNumber || t.referenceNumber;

            if (type === 'AR' && invoiceNo) {
                const sd = sdMap.get(invoiceNo);
                if (sd) {
                    entityName = sd.buyerName || sd.recipient;
                    documentTotal = Number(sd.grandTotal || 0);
                    if (sd.isVoid) status = 'VOID';
                }
            } else if (type === 'AP' && invoiceNo) {
                const gr = grMap.get(invoiceNo);
                if (gr) {
            ...
            return {
                id: t.id,
                date: t.date,
                documentNumber: invoiceNo || t.referenceNumber || '-',
                type,
                entityName,
