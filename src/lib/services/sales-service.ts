...
export async function createSalesDeliveryService(data: any, userId: string) {
    ...
    return await prisma.$transaction(async (tx: any) => {
        ...
        const totalDiscountNominal = Number(data.totalDiscount) || 0;
    ...
    return await prisma.$transaction(async (tx: any) => {
        ...
        if (updatedDelivery) {
            for (const sdItem of updatedDelivery.items) {
                ...
                // Match with data.items to get selectedLotId
                const inputItem = data.items.find((i: any) => i.productId === sdItem.productId && !i._allocatedLotFlagUpdate);
                if (inputItem) {
                    inputItem._allocatedLotFlagUpdate = true;
                }
...
export async function createSalesOrderService(data: any, userId: string) {
    ...
    return await prisma.$transaction(async (tx: any) => {
        ...
        const subtotalExact = data.items.reduce((acc: number, i: any) => acc + (Number(i.quantity) * Number(i.salesPrice)) - Number(i.discount || 0), 0);
        const totalDiscountNominal = Number(data.totalDiscount) || 0;
        const taxRatePercent = Number(data.taxRate) || 0;
